...
% This script creates just makes a single example gif of Antarctic coastal 
% change 1997-2021. For larsens. 
% 
...
txt = text(xc,yc,D.name,'horiz','center','vert','middle',...
   'fontsize',7,'color',.2*[1 1 1],'fontangle','italic','clipping','on');
...
figure('pos',[10.00        477.00        580.00        475.00])

hh = imagescn(x,y,ice(:,:,1));
hold on
axis tight off
hb = bedmachine('gl','color',.5*[1 1 1],'LineWidth',0.25); 
ttl = ntitle(num2str(floor(year(1))),'fontweight','bold','color',.6*[1 1 1],'FontSize',14); 
col = cmocean('ice'); 
set(gcf,'color',col(1,:))
cmocean ice
...
tcb = textcolorbar(unique(floor(year)),'location','ne'); 
gif('test.gif','DelayTime',1/4,'resolution',300)

for k=1:2
   gif % repeat the first framea . few times 
end
   for k = 1:24
      hh.CData = ice(:,:,k); 
